...
def draw_menu(context: Context, menu: str) -> Dict[str, Any]:
    ...
    try:
        ...
        items = MenuItems.objects.filter(menu__title=menu)
        ...
        result_dict = {
            'items': [
                item for item in MenuItems.objects.filter(menu__title=menu, parent=None).values()
            ]
        }
...
def get_selected_item_id_list(parent: MenuItems, primary_item: List[MenuItems], selected_item_id: int) -> List[int]:
    """
